...
def assets_file_info(file_handler):
    ...
    # Metadata - Dict
    metadata = {}
    log.info('metadata start: 0x%08X', f.tell())
    metadata['version']          = read_null_term_string(f)
    ...
    typetree = {}
    log.info('type tree start: 0x%08X', f.tell())
    for x in range(0, metadata['base_class_count']):
        typeroot = {}
        typeroot['class_id'] = read('<i', 4, f)
    ...
    log.info('object info table start: 0x%08X', f.tell())
    ...
    log.info('object id table start: 0x%08X', f.tell())
    ...
    # File Id Table - List
    log.info('file id table start: 0x%08X', f.tell())
    ...
    # Entry Table - List
    log.info('entry table start: 0x%08X', f.tell())
    entries = []
...
def assets_entry(file_handler, entry, duplicate=False): # duplicate option added for error recovery - what did i mean by this???
    ...
    if type_id in (28, 213):
        ...
        if type_id == 28:
            ...
            node['ress_name']  = read_string(data)
            align(4, data)
            log.info('image format: %i', node['img_format'])
